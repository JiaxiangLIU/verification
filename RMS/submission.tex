...
fails in Scenario~(iv).  One counterexample of the schedulability for
...
when an interrupt request is performed, two different behaviors are
possible: (i) the system performs task switching, during which the
interrupt request is masked, hence the scheduling may be delayed; (ii)
the system answers the interrupt request immediately, the switching
being delayed. Tackling such non-determinism via theoretical analyses
seems more complicated than our automatic approach.
...
with different languages and tools. The RMS algorithm is analyzed

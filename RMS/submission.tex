...
\item $\Sigma$ is an algebraic \emph{signature}, that is, a set of
  ...
  \hl{\emph{Terms} are expressions built from function symbols and
  variables.}
\item $(\Sigma, E\cup A)$ is a \emph{membership equational logic
  ...
  A)$ models the system's ``static'' states \hl{as terms of some sort}, and
...
\item $IR$ is a set of \emph{labeled conditional rewrite rules}
  ...
  \emph{label}, \hl{$t,t'$ are terms}. Such a rule specifies an
...
$val_i$ is the value of the attribute $att_i$ with $i\in [1,n]$. \hl{Rules
...
\section{\hl{Formal Modeling of the Implementation}}
\label{s:formalism}
...
$C_3=4.5ms$.
\end{itemize}
\hl{Note that thanks to the powerful expressiveness of Real-Time Maude, we
only need to define an initial state of sort} \verb|System| \hl{to specify
...
\hl{The results above have demonstrated that our approach is capable of
...
sampling strategy, \hl{for a large class of real-time systems possessing a
...
tick-invariant, then using Theorem~\ref{t:completeness}. \hl{For a more
...
target implementation is not in their scope. \hl{Furthermore, compared
...
\hl{\cite{TianD2011} and~\cite{DBLP:conf/iceccs/CuiDT14} also made use of
...
\appendix[\hl{Proof of Theorem~\ref{t:main}}]
\label{a:proof}

...
scheduling algorithms. There are large numbers of results about RMS,
especially on its schedulability. However, these theoretical results
contain not enough details to be used directly to investigate a
realistic RMS implementation. On the other hand, the correctness of
such an implementation is of the same importance. In this paper, we
analyze a realistic RMS implementation by formal modeling using
Real-Time Maude, a language and tool based on rewriting
logic. Overhead and some details of the hardware are taken into
account in the model. We verify the schedulability and the correctness
of the implementation within several scenarios, using model checking
tool built in Real-Time Maude. The soundness and the completeness of
our results are proved in the end.
\end{abstract}
...
\end{tikzpicture}
\caption{An counterexample of schedulablity in Scenario~(iv). Each job
...
and modeling some mechanism of the hardware, we believe that our model
contains enough details to be analyzed for the behaviors of the real
system. Two important properties--schedulability and correctness--are
verified by model checking on our model, within several given
scenarios. We also prove at last the soundness and completeness of our
results.

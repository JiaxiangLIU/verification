...
It is also crucial to ensure the reliability of the implementation of
RMS when it serves in a safety-critical system. When analyzing a
realistic implementation instead of the algorithm, theoretical results
may be no more applicable. For instance, switching overhead may lead
to unschedulability even if the task set satisfies the schedulability
condition derived from algorithm analysis. Extensive effort to apply
...
this standard, ideal setting. Assumption~(A1) will be modified because
of the interrupt masking mechanism, while (A2) and (A4) are relaxed to
obtain a more realistic analysis model.

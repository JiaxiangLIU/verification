...
broken by overhead in the system, or by the interrupt masking
mechanism which may delay interrupt handling. On the other hand,
correctness of the implementation with respect to the algorithm is
difficult to be verified by the traditional methods such as testing
and simulation, since they are incomplete. Extensive effort to apply
formal methods, such as model checking and theorem proving, has been
made to analyze safety-critical systems for the past few
...
masking mechanism, while (A2) and (A4) are relaxed to obtain a more
...
occurs, if the system is interruptable, i.e. the interrupt masking bit
is cleared, the handler function $schedule()$ will be invoked;
otherwise $schedule()$ will be pending until the interrupt masking bit
...
In Figure~\ref{a:schedule}, the handler function $scheduler()$ first
updates status of all tasks in $taskList$ via function
$updateStatus()$. This updating actually initiates tasks which should
be scheduled in the current interrupt cycle. Then $scheduler()$
...
disabled (the interrupt masking bit is set). Due to the interrupt
masking bit, its execution cannot be interrupted when it is updating
...
masking mechanism, we model the implementation under the following
assumptions:
...
finishes and the interrupt masking bit is cleared, which is different
...
because switching is performed and the interrupt masking bit is set at
time $5$.
...
\subfloat[RMS Algorithm]{
  ...
  \end{scope}
    ...
    % time indicator
    \draw [time line] (1.5,0.7) -- (1.5,0);
    \draw (1.5,0) node [below] {$3$} -- +(0,2pt);
    \draw [time line] (2.5,0.7) -- (2.5,0);
    \draw (2.5,0) node [below] {$5$} -- +(0,2pt);
    \draw [time line] (4,0.7) -- (4,0);
    \draw (4,0) node [below] {$8$} -- +(0,2pt);
    \draw [time line] (4.5,0.3) -- (4.5,0);
    \draw (4.5,0) node [below] {$9$} -- +(0,2pt);
    \draw [time line] (5,0.7) -- (5,0);
    \draw (5,0) node [below] {$10$} -- +(0,2pt);
    
    % tasks
    \draw [periodic task] (0,0.7) rectangle +(1.5,0.14);
    \draw [periodic task] (1.5,0.3) rectangle +(1,0.14);
    \draw [periodic task] (2.5,0.7) rectangle +(1.5,0.14);
    \draw [periodic task] (4,0.3) rectangle +(0.5,0.14);
    ...
    % completion of tasks
    \draw [complete] (1.5,0.84) -- +(0,0.01);
    \draw [complete] (4,0.84) -- +(0,0.01);
    \draw [complete] (4.5,0.44) -- +(0,0.01);
  \end{tikzpicture}
\label{sf:rmsalg}}\\
\subfloat[RMS Implementation]{
  \begin{tikzpicture}[font=\small,scale=1.25,
    ...
    \draw (0.5,0) node [below] {$1$} -- +(0,2pt);
    \draw [time line] (1.25,1.5) -- (1.25,0);
    \draw (1.25,0) node [below] {$2.5$} -- +(0,2pt);
    \draw [time line] (1.75,1.1) -- (1.75,0);
    \draw (1.75,0) node [below] {$3.5$} -- +(0,2pt);
    \draw [time line] (2.25,1.1) -- (2.25,0);
    \draw (2.25,0) node [below] {$4.5$} -- +(0,2pt);
    \draw [time line] (2.75,1.5) -- (2.75,0);
    \draw (2.75,0) node [below] {$5.5$} -- +(0,2pt);
    \draw [time line] (3.25,1.5) -- (3.25,0);
    \draw (3.25,0) node [below] {$6.5$} -- +(0,2pt);
    \draw [time line] (4.25,1.5) -- (4.25,0);
    \draw (4.25,0) node [below] {$8.5$} -- +(0,2pt);
    \draw [time line] (4.75,0.3) -- (4.75,0);
    \draw (4.75,0) -- +(0,2pt);
    \draw (4.65,0) node [below] {$9.5$};
    ...
    \draw [periodic task] (3.25,1.5) rectangle +(1,0.14);
    \draw [system task] (4.25,0.3) rectangle +(0.5,0.14);
    ...
    \draw [complete] (4.25,1.64) -- +(0,0.01);
  \end{tikzpicture}
\label{sf:rmsimp}}
...
\begin{tikzpicture}[font=\small,
  ...
  \draw (0,2) node [left] {task$2$};
  \draw (0,2.7) node [left] {task$1$};
  \draw (0,3.4) node [left] {task$0$};

...
#define CRC_LEN 2
#define MIN_PACKE_CMD (9 + 5 + CRC_LEN) //����֡ + 5��0 + ����CRC
...
static int recv_check(char *dest_data, int *dest_len, char *src_data, int src_len)
{
	...
	*dest_len = ERROR_CODE_PARAMETER;
    ...
    if (src_data[SOCKET_HEADR_INDEX] != 0x5a) { //Э��ͷ
...
static struct socket_t loc_sockets[MAX_SOCKET_NUM]; //���socket_fd����Ӧ���ȼ�
...
static int socket_register(int socket_fd, uint8_t type)
{
    ...
    snd_buf[0] = 0x5a;  //��ݵײ�ͨ��Э��ȷ��ע��֡�ĸ�ʽ������
    snd_buf[1] = 0x51;
    snd_buf[2] = type;
    ...
    if (rcv_buf[0] == 0x5a && rcv_buf[1] == 0x50 && rcv_buf[2] == (type + 3))   //��ȷ����ֵ
        return 0;
    else  {  //ע��ʧ��
    	log_error("to_vote error: %s\n", error_code_name(ERROR_CODE_SOCKET_REGISTER));

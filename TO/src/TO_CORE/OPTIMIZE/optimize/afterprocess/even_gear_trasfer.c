/**
 * @file even_gear_trasfer.c
 * @brief 临时限速最后同原始曲线交叉点档位连续变换策略，其中中间档位为奇数个档位。如从5档-2档，中间只有4,3档，偶数个档位
...
OPTSCH*  even_gear_trasfer(OPTSCH* new_tmp_curve,int * tmp_final_len,int* cap,OPTSCH* optSch,int *srcInxT,float tStep,float idelStep,OPTIMIZE_RESULT* opt_ret)
{
	...
	char desc[EXP_DESC_LENGTH]="#even_gear_trasfer";
	float delta_t=0;
	...
	if(eGear-sGear>0)  //档位是递增趋势
	{
		...
		{
			printf("even_gear_trasfer Error\n");
		}
	}
	else //档位是递减趋势,若没有焦点，则需要把档位往前移（index增大的方向）
	{
		midGear=sGear+(eGear-sGear)/2;
		float needTime=0;
		for(i=sGear-1;i>=midGear;i--)
		...
		if(flag==0)
		{
			if(index_back!=-1)
			{
				...
				//				i=srcInx;
				//				while ( index_back<=ln){  //原始曲线部分
				//					optSch[i].start = optBack[index_back].start;
				//					optSch[i].gear = optBack[index_back].gear;

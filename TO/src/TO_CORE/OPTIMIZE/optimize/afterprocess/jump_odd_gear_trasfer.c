...
OPTSCH* jump_odd_gear_trasfer(OPTSCH* new_tmp_curve,int * tmp_final_len,int* cap, OPTSCH* optSch,int *srcInxT,float tStep,float idelStep,OPTIMIZE_RESULT* opt_ret)
{
	...
	char desc[EXP_DESC_LENGTH]="#jump_odd_gear_trasfer";
	float delta_t=0;
	...
	if(eGear-sGear>0)  //档位是递增趋势
	{
		...
		while(flag!=0)//递增的档位若没有焦点，则需要把档位往后移（index减小的方向）
		{
			...
			for(;gear<=eGear;gear++)
			{
				...
				while(1)
				{
					...
					if(gear==0)
					{
						if(gearTime>idelStep)//变换档位(当前档位为惰行)
						{
							break;
						}
					}
					...
					{
						if(gearTime>tStep)//变换档位(当前档位为非惰行档位)
						{
							break;
						}
					}
		...
		{
			printf("jump_odd_gear_trasfer Error\n");
		}
	}
	else //档位是递减趋势
	{

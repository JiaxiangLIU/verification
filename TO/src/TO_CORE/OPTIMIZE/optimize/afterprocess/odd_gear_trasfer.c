/**
 * @file odd_gear_trasfer.c
 *@brief 临时限速最后同原始曲线交叉点档位连续变换策略，其中中间档位为奇数个档位。如从5档-3档，中间只有4档，奇数个档位
...
OPTSCH*  odd_gear_trasfer(OPTSCH* new_tmp_curve,int * tmp_final_len,int *cap,OPTSCH* optSch,int* srcInxT,float tStep,float idelStep,OPTIMIZE_RESULT* opt_ret)
{
	...
	char desc[EXP_DESC_LENGTH]="#odd_gear_trasfer";
	float delta_t=0;
	...
	if(eGear-sGear>0)  //档位是递增趋势
	{
		 ...
		 */
		while(flag!=0)//递增的档位若没有交点，则需要把档位往后移（index减小的方向）
		{
			if(flag==1) //追不上,集体往后移
			{
			...
			for(;gear<=eGear;gear++)
			{
				...
				while(1)
				{
					...
					v = v+delta_v;
					if(v>opt_const.LIMITV || v<0) //超出限速
					{
						flag=-2;
						break;
					}

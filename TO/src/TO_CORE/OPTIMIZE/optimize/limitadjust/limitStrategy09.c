...
OPTIMIZE_RESULT limitStrategy09(OPTSCH* optsch,int len, LIMITINFO limitInfo,int count,int flagS,int longflag, float srcStart)
{
	...
	//异常处理描述
	char desc[EXP_DESC_LENGTH]="#limitStrategy09";
	OPTIMIZE_RESULT ret;
...
//	float t;
	...
	//陡上坡，必须通过最大档位反求到两个坡段的交汇点
	indexTemp = index2 - 1;
	v = optsch[index2].velocity;
	headX = optsch[index2].start;
	int index_back=-1;
	index=indexTemp;
	...
	while(1)
	{
		...
		if(delta_v>0 || gear==opt_const.MAXGEAR)
			break;
		else
			gear=gear+1;
	}
	...
	while(1)
	{
		...
		v = v-delta_v;
		headX = headX -s;
		if (v > value && (headX > optsch[indexM].start || fabs(headX -  optsch[indexM].start) < opt_const.PRECISION)) //在还没有反求到两个坡段类型交汇点时就超出限速，此时需要升档位
		{
		...
		if (headX < optsch[indexM].start || fabs(headX - optsch[indexM].start) < opt_const.PRECISION)
			break;
	...
	while(1)
	{
		...
		if(delta_v<0 || gear==opt_const.MINGEAR)
			break;
		else
			gear=gear-1;
	}
	...
	{
		int j ;
		float deltaV=optsch[index1].velocity-optsch[i].velocity;
		float deltaS=optsch[i].start-optsch[index1].start;
		for (j=index1+1;j<=i;j++)
		{
			float del_s=optsch[j].start-optsch[j-1].start;
			float del_s1=optsch[j+1].start-optsch[j].start;
			optsch[j].gear=identifyGear(optsch, j, opt_const.AIRE); //提醒空气制动标识
			optsch[j].velocity=optsch[j-1].velocity-del_s*deltaV/deltaS;//速度为最大限速
//			float time=del_s1*3.6/optsch[j].velocity;
			optsch[j].time=del_s1*3.6/optsch[j].velocity;
		}

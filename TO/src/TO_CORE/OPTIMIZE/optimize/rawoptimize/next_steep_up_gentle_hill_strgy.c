/**
 * @file      next_steep_up_gentle_hill_strgy.c
 * @brief  缓坡策略  破段类型标记为flag=-1/0/1  用于下一段为陡上坡的缓坡策略
 * @date  2014.12.10
 * @author sinlly
 */
...
OPTSCH* next_steep_up_gentle_hill_strgy(float vo, float v_interval, int count, int roadIndex,int index, OPTSCH* optSch, float avgV,OPTIMIZE_RESULT *ret) {
	int tempIndex, gear, flag, gearup, geardown,index_back;
	float headX, v,currentV;
	OPTSCH* optBack;
	//异常处理标识符
	char  tmp[EXP_DESC_LENGTH];
	sprintf(tmp,"%d",roadIndex);
	char desc[EXP_DESC_LENGTH]="#next_steep_up_gentle_hill_strgy;rcIndex=";
	strcat(desc,tmp);
	...
	if (v > avgV + 5.0 || fabs(v - (avgV + 5.0))<opt_const.PRECISION) { //起始速度较高，先惰行至均速附近，再按均速牵引
		flag = 0;
		gear = 0;
		fopt_ret=gr_trac_to_avgv_till_ed_no_db_gear(gear,  avgV,  roadIndex, v_interval, &index,&count,  &flag, &optSch);
...
//			ret=&fopt_ret;
		...
		//设定最大上下浮速度区间
		fopt_ret=get_avg_up_down_gear_no_db_gear(&headX,&v,  avgV,&count,&flag,&gearup,&geardown);
		if(fopt_ret.opt_rest!=OPTIMIZE_SUCCESS)
		{
			strcat(desc,fopt_ret.description);
			optimize_exception_decorator(fopt_ret.opt_rest,&fopt_ret,desc);
//			ret=&fopt_ret;
		...
		fopt_ret=constv_gr_down_up_til_end_no_db_gear(headX, v,  gearup, geardown, roadIndex,&index,&count,  &flag, v_interval,  avgV, &optSch);
		if(fopt_ret.opt_rest!=OPTIMIZE_SUCCESS)
		{
			strcat(desc,fopt_ret.description);
			optimize_exception_decorator(fopt_ret.opt_rest,&fopt_ret,desc);

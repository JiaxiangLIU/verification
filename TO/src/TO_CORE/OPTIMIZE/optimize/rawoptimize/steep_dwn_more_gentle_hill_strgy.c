/**
 * @file     steep_dwn_more_gentle_hill_strgy.c
 * @brief  缓坡策略  破段类型标记为flag=-1/0/1，其中下坡占的百分比大于24
 * @date  2014.4.9
 * @author sinlly
 */
...
OPTSCH* steep_dwn_more_gentle_hill_strgy(float vo, float v_interval, int count, int roadIndex,int index, OPTSCH* optSch, float avgV,OPTIMIZE_RESULT *ret) {
	...
	char desc[EXP_DESC_LENGTH]="#steep_dwn_more_gentle_hill_strgy;rcIndex=";
	strcat(desc,tmp);
	...
	//原程序count=10取消
	if (v >= avgV + 5) { //起始速度较高，先惰行至均速附近，再按均速牵引
		flag = 0;
		gear = -3;
...
//			ret=&fopt_ret;
	...
	} else if (v <= avgV - 5) //起始速度较低，则牵引到匀速附近，再按匀速牵引
	{
		flag = -1;
...
//			ret=&fopt_ret;
		...
		//		gear=-2;
		//		 gr_trac_til_ed(gear, roadIndex, &index,&count, &optSch);
	}


	//下一段是上坡，需要最大加速到下一段入口速度附近
	if (optSch[index - 1].start >= rc[roadIndex].end) {
...
//				ret=&fopt_ret;
	...
	else if (roadIndex != (MAX_ROADCATEGORY - 1) && rc[roadIndex].flag == -3

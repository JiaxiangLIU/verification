/**
 * @file     steep_dwn_Strgy.c
 * @brief  陡下坡策略  坡段类型标记为flag=-2
 * @date  2014.4.9
 * @author sinlly
 */
...
OPTSCH* steep_dwn_strgy(float vo, float v_interval, int count, int roadIndex,int index, OPTSCH* optSch, float avgV,OPTIMIZE_RESULT *ret) {
	...
	char desc[EXP_DESC_LENGTH]="#steep_dwn_strgy;rcIndex=";
	strcat(desc,tmp);
	...
	if (v > vo) { //用微减速状态至期望速度
		gear=0;
		fopt_ret=db_dyn_gr_til_vo_or_ed_or_lmt(gear, vo,roadIndex , &index, &count, opt_const.LIMITV, &flag,&optSch);
...
//			ret=&fopt_ret;
		...
		gear=0;
		fopt_ret=dyn_gr_trac_til_vo_or_ed_or_lmt(gear, vo,roadIndex , &index, &count, opt_const.MINV, &flag,&optSch);
		if(fopt_ret.opt_rest!=OPTIMIZE_SUCCESS)
		{
			strcat(desc,fopt_ret.description);
			optimize_exception_decorator(fopt_ret.opt_rest,&fopt_ret,desc);
//			ret=&fopt_ret;
	...
	if (flag == 1 || flag == 2) { //已到达输出速度且未走完全程
		...
		while(isOK!=TRUE  && gear!=opt_const.MINGEAR ) //制动是否能够达到要求
		{
			gear = gear-1;
			isOK=TRUE;
			index=srcIndex;

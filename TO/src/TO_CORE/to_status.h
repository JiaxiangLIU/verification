...
/* changelog：ver1.300 2015-11-11 添加交路数据接收应答事件 */
...
//typedef enum {
	...
	EVENT_ROUTE_DATA_SEND_ACK = 0x06, /* 交路数据版本不一致处理：交路提取完成并发送：坡度应答 */
	EVENT_ROUTE_DATA_CURVE_ACK = 0x07, /* 交路数据版本不一致处理：发送：曲线应答  */
	EVENT_ROUTE_DATA_LIMIT_ACK = 0x08, /* 交路数据版本不一致处理：发送：限速应答  */
	EVENT_ROUTE_DATA_TELESEME_ACK = 0x09, /* 交路数据版本不一致处理：发送：信号机应答  */
	EVENT_ROUTE_DATA_TUNNEL_ACK = 0x0A, /* 交路数据版本不一致处理：发送：隧道应答  */
	EVENT_ROUTE_DATA_STATION_ACK = 0x0B, /* 交路数据版本不一致处理：发送：车站应答  */

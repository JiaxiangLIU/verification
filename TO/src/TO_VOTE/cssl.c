		  ...
		  int stop)
{
	...
	foe.type = F_OWNER_TID;
	foe.pid = syscall(SYS_gettid);
	serial->fd=open(fname,O_RDWR|O_NOCTTY|O_NONBLOCK);
	fcntl(serial->fd,F_SETSIG,CSSL_SIGNAL);
	//fcntl(serial->fd,F_SETOWN,syscall(SYS_gettid));
	fcntl(serial->fd, F_SETOWN_EX, &foe);
		   ...
		   int stop)
{
    ...
    int high_speed_flag = 0; /* Add flag for 921600 */
    ...
    /* particular setting for baudrate 912600 */
//    if (high_speed_flag == 1)
 //   	setbaudrate(serial->fd, 912600);
    switch (high_speed_flag) {
	case 1:
		setbaudrate(serial->fd, 460800);
		break;
	case 2:
		setbaudrate(serial->fd, 921600);
		break;
	default:
		break;
    }

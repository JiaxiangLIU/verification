...
//			break;
           case EVENT_TRIP_INIT_START:
			      threadpool_add(pool, &route_init_start, event, 0);
			      break;
           //case EVENT_TRIP_STATUS_NOTICE:
          }
//	}
//	if(event->source==VOTE_EVENT){
//		CTRBOX_CMD* cmd=(CTRBOX_CMD*)malloc(sizeof(CTRBOX_CMD));
//		switch(event->type){
//
//		    case EVENT_DISP_MANNAL:
//		    	  *cmd=CTRBOX_CMD_EVENT_DISP_MANNAL ;
//		    	  threadpool_add(pool, &ctrbox_comm,cmd , 0);
//		    	  event_delete(event);
//		    	  break;
//		    case EVENT_AUTO_COMPLETE:
//		    	  *cmd=CTRBOX_CMD_EVENT_AUTO_COMPLETE;
//		    	   threadpool_add(pool, &ctrbox_comm,cmd , 0);
//		    	   event_delete(event);
//		    	   break;
//		    case EVENT_DISP_AUTO:
//		    	*cmd=CTRBOX_CMD_EVENT_DISP_AUTO;
//		    	  threadpool_add(pool, &ctrbox_comm,cmd , 0);
//		    	  event_delete(event);
//		    	  break;
//		    case EVENT_MANNAL_COMPLETE:
//		    	*cmd=CTRBOX_CMD_EVENT_MANNAL_COMPLETE;
//		    	   threadpool_add(pool, &ctrbox_comm,cmd, 0);
//		    	   event_delete(event);
//		    	   break;
//		    case EVENT_TWO_GEAR_CONFIRM:
//		    	*cmd=CTRBOX_CMD_EVENT_TWO_GEAR_CONFIRM;
//		    	   threadpool_add(pool, &ctrbox_comm,cmd, 0);
//		    	   event_delete(event);
//		    	   break;
//		}
